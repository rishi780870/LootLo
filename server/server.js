...
app.get("/history", async (req, res) => {
  try {
    const snap = await db.collection("wingoHistory").orderBy("createdAt", "desc").limit(20).get();
    res.json(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
function calcPayout(bet, result) {
  ...
  if (bet.type === "number" && Number(bet.value) === result) { won = true; multiplier = 9; }
  else if (bet.type === "color" && bet.value === "Green" && color === "Green") { won = true; multiplier = 2; }
  else if (bet.type === "color" && bet.value === "Red" && color === "Red") { won = true; multiplier = 2; }
  else if (bet.type === "color" && bet.value === "Violet" && color === "Violet") { won = true; multiplier = 4.5; }
  else if (bet.type === "size" && bet.value === "Big" && size === "Big") { won = true; multiplier = 2; }
  else if (bet.type === "size" && bet.value === "Small" && size === "Small") { won = true; multiplier = 2; }
  return { won, payout: won ? Math.floor(bet.amount * multiplier) : 0 };
}

// ── KEY FIX: isProcessing prevents duplicate processing ───────────────
...
setInterval(async () => {
  ...
  try {
    ...
    if (phase === "betting" && elapsed >= 30) {
      // KEY FIX: period-level dedup — same period never processed twice
      if (lastProcessedPeriod === game.period) return;

      const result = Math.floor(Math.random() * 10);
      const { color, size } = getColorInfo(result);
      console.log(`\n=== Period ${game.period} | Result: ${result} (${color}, ${size}) ===`);

      // STEP 1: Update gameState to result phase IMMEDIATELY
      // This ensures frontend gets correct result from gameState.result
      await gameRef.update({
        phase: "result",
        result,
        resultColor: color,
      ...
      // Mark processed right after gameState update
      lastProcessedPeriod = game.period;

      // STEP 2: Settle bets
      const betsSnap = await db.collection("userBets")
        .where("status", "==", "pending")
        .where("period", "==", game.period)
        .get();
      ...
      for (const betDoc of betsSnap.docs) {
        ...
        batch.update(betDoc.ref, {
          status: won ? "won" : "lost",
          result, resultColor: color, resultSize: size,
          payout, settledAt: Date.now(),
        });
        if (!userUpdates[bet.uid]) userUpdates[bet.uid] = { balanceDelta: 0, wins: 0, losses: 0, winningsDelta: 0 };
        if (won) {
          userUpdates[bet.uid].balanceDelta += payout;
          userUpdates[bet.uid].winningsDelta += payout;
          userUpdates[bet.uid].wins += 1;
      ...
      // STEP 4: Save wingoHistory ONCE — check for duplicate first
      const existingHistory = await db.collection("wingoHistory")
        .where("period", "==", game.period)
        .limit(1)
        .get();
      ...
      if (existingHistory.empty) {
        ...
        console.log(`📝 History saved for period ${game.period}`);
      } else {
        console.log(`⚠️ Duplicate prevented for period ${game.period}`);
      }

      await updateLeaderboard();
      console.log(`✅ Period ${game.period} done.`);
    }
...
async function updateLeaderboard() {
  try {
    const betsSnap = await db.collection("userBets").where("status", "==", "won").get();
    const totals = {};
    betsSnap.docs.forEach((d) => {
      const b = d.data();
      if (!b.uid) return;
      if (!totals[b.uid]) totals[b.uid] = { uid: b.uid, totalWon: 0, name: b.userName || "Player" };
      totals[b.uid].totalWon += b.payout || 0;
    });
    const sorted = Object.values(totals).sort((a, b) => b.totalWon - a.totalWon).slice(0, 10);
    await db.collection("leaderboard").doc("wingo").set({ top10: sorted, updatedAt: Date.now() });
