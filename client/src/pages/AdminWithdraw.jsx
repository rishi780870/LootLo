...
export default function AdminWithdraw() {
  ...
  const approveWithdraw = async (item) => {
    ...
    try {
      await updateDoc(doc(db, "withdrawRequests", item.id), { status: "approved" });
      alert("Approved ✅");
      loadRequests();
    } catch (e) {
      console.log(e);
  ...
  const rejectWithdraw = async (item) => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        await updateDoc(userRef, { balance: (userData.balance || 0) + item.amount });
      }
      await updateDoc(doc(db, "withdrawRequests", item.id), { status: "rejected" });
    ...
    <div style={styles.page}>
      ...
      <div style={styles.container}>
        <div style={styles.header}>
          ...
          <h1 style={styles.title}>💸 Withdraw Requests</h1>
          <button onClick={loadRequests} className="refreshBtn">🔄</button>
        </div>

        <div style={styles.tabRow}>
          <button onClick={() => setTab("pending")} className={tab === "pending" ? "tabActive" : "tab"}>
            ⏳ Pending
            {pending.length > 0 && <span className="tabBadge">{pending.length}</span>}
          </button>
          <button onClick={() => setTab("history")} className={tab === "history" ? "tabActive" : "tab"}>
            📜 History
            ...
            <div key={item.id} style={styles.card} className="reqCard">
              ...
              <div style={styles.cardDetails}>
                <div style={styles.detailItem}>
                  ...
                  <span style={styles.detailValue}>🏦 {item.upiId}</span>
                </div>
                <div style={styles.detailItem}>
              ...
              {tab === "pending" ? (
                <div style={styles.btnRow}>
                  ...
                  <button
                    ...
                    className="rejectBtn"
                  >
                    {actionLoading === item.id + "_reject" ? "..." : "❌ Reject & Refund"}
                  </button>
                </div>
              ) : (
                <div style={{
...
const styles = {
  ...
  glow: {
    position: "fixed", top: -100, left: "20%", width: 300, height: 300,
...
const css = `
  ...
  .tabActive {
    ...
    color: #f59e0b; font-size: 14px; font-weight: 700; cursor: pointer;
    display: flex; align-items: center; justify-content: center; gap: 8px;
  }
  .tabBadge {
  ...
  .reqCard:hover {
    border-color: rgba(245,158,11,0.25); transform: translateY(-1px);
